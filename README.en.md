...
-   [What's happened_"collection"_?](#Что-такое-коллекция)
-   [Name the main JCF interfaces and their implementations.](#Назовите-основные-интерфейсы-jcf-и-их-реализации)
-   [Arrange the following interfaces in a hierarchy:`List`,`Set`,`Map`,`SortedSet`,`SortedMap`,`Collection`,`Iterable`,`Iterator`,`NavigableSet`,`NavigableMap`.](#Расположите-в-виде-иерархии-следующие-интерфейсы-list-set-map-sortedset-sortedmap-collection-iterable-iterator-navigableset-navigablemap)
-   [Why`Map`- this is not`Collection`, while`List`And`Set`are`Collection`?](#Почему-map--это-не-collection-в-то-время-как-list-и-set-являются-collection)
-   [What is the difference between the classes`java.util.Collection`And`java.util.Collections`?](#В-чем-разница-между-классами-javautilcollection-и-javautilcollections)
-   [What is “fail-fast behavior”?](#Что-такое-fail-fast-поведение)
-   [What's the difference between fail-fast and fail-safe?](#Какая-разница-между-fail-fast-и-fail-safe)
-   [Give examples of iterators that implement fail-safe behavior](#Приведите-примеры-итераторов-реализующих-поведение-fail-safe)
-   [How are they different?`Enumeration`And`Iterator`.](#Чем-различаются-enumeration-и-iterator)
-   [How are they connected?`Iterable`And`Iterator`?](#Как-между-собой-связаны-iterable-и-iterator)
-   [How are they connected?`Iterable`,`Iterator`и «for-each»?](#Как-между-собой-связаны-iterable-iterator-и-for-each)
-   [Compare`Iterator`And`ListIterator`.](#Сравните-iterator-и-listiterator)
-   [What happens when called`Iterator.next()`without prior call`Iterator.hasNext()`?](#Что-произойдет-при-вызове-iteratornext-без-предварительного-вызова-iteratorhasnext)
-   [How many elements will be skipped if`Iterator.next()`will be called after 10 calls`Iterator.hasNext()`?](#Сколько-элементов-будет-пропущено-если-iteratornext-будет-вызван-после-10-ти-вызовов-iteratorhasnext)
-   [How will the collection behave if you call`iterator.remove()`?](#Как-поведёт-себя-коллекция-если-вызвать-iteratorremove)
-   [How will an already instantiated iterator behave for`collection`, if called`collection.remove()`?](#Как-поведёт-себя-уже-инстанциированный-итератор-для-collection-если-вызвать-collectionremove)
-   [How to avoid`ConcurrentModificationException`while iterating through a collection?](#Как-избежать-concurrentmodificationexception-во-время-перебора-коллекции)
-   [Which collection implements the FIFO service discipline?](#Какая-коллекция-реализует-дисциплину-обслуживания-fifo)
-   [Which collection implements the FILO service discipline?](#Какая-коллекция-реализует-дисциплину-обслуживания-filo)
-   [How is it different?`ArrayList`from`Vector`?](#Чем-отличается-arraylist-от-vector)
-   [Why did they add it?`ArrayList`, if you've already been`Vector`?](#Зачем-добавили-arraylist-если-уже-был-vector)
-   [How is it different?`ArrayList`from`LinkedList`? In what cases is it better to use the first one, and in what cases the second one?](#Чем-отличается-arraylist-от-linkedlist-В-каких-случаях-лучше-использовать-первый-а-в-каких-второй)
-   [What works faster?`ArrayList`or`LinkedList`?](#Что-работает-быстрее-arraylist-или-linkedlist)
-   [What is the worst running time of the method`contains()`for an element that is in`LinkedList`?](#Какое-худшее-время-работы-метода-contains-для-элемента-который-есть-в-linkedlist)
-   [What is the worst running time of the method`contains()`for an element that is in`ArrayList`?](#Какое-худшее-время-работы-метода-contains-для-элемента-который-есть-в-arraylist)
-   [What is the worst running time of the method`add()`for`LinkedList`?](#Какое-худшее-время-работы-метода-add-для-linkedlist)
-   [What is the worst running time of the method`add()`for`ArrayList`?](#Какое-худшее-время-работы-метода-add-для-arraylist)
-   [Need to add 1 million elements, what structure are you using?](#Необходимо-добавить-1-млн-элементов-какую-структуру-вы-используете)
-   [How to remove elements from`ArrayList`? How does the size change in this case?`ArrayList`?](#Как-происходит-удаление-элементов-из-arraylist-Как-меняется-в-этом-случае-размер-arraylist)
-   [Propose an efficient algorithm for removing several adjacent elements from the middle of a list, implemented by`ArrayList`.](#Предложите-эффективный-алгоритм-удаления-нескольких-рядом-стоящих-элементов-из-середины-списка-реализуемого-arraylist)
-   [How much additional memory is needed when calling`ArrayList.add()`?](#Сколько-необходимо-дополнительной-памяти-при-вызове-arraylistadd)
-   [How much additional memory is allocated when calling`LinkedList.add()`?](#Сколько-выделяется-дополнительно-памяти-при-вызове-linkedlistadd)
-   [Estimate the amount of memory for storing one primitive type`byte`V`LinkedList`?](#Оцените-количество-памяти-на-хранение-одного-примитива-типа-byte-в-linkedlist)
-   [Estimate the amount of memory for storing one primitive type`byte`V`ArrayList`?](#Оцените-количество-памяти-на-хранение-одного-примитива-типа-byte-в-arraylist)
-   [For`ArrayList`or for`LinkedList`operation of adding an element to the middle (`list.add(list.size()/2, newElement)`) slower?](#Для-arraylist-или-для-linkedlist-операция-добавления-элемента-в-середину-listaddlistsize2-newelement-медленнее)
...
-   [Compare interfaces`Queue`And`Deque`.](#Сравните-интерфейсы-queue-и-deque)
-   [Who is expanding who?`Queue`expands`Deque`, or`Deque`expands`Queue`?](#Кто-кого-расширяет-queue-расширяет-deque-или-deque-расширяет-queue)
-   [Why`LinkedList`implements and`List`, And`Deque`?](#Почему-linkedlist-реализует-и-list-и-deque)
-   [`LinkedList`Is it a singly, doubly, or four linked list?](#linkedlist--это-односвязный-двусвязный-или-четырехсвязный-список)
-   [How to iterate over elements`LinkedList`in reverse order without using slow`get(index)`?](#Как-перебрать-элементы-linkedlist-в-обратном-порядке-не-используя-медленный-getindex)
-   [What it allows you to do`PriorityQueue`?](#Что-позволяет-сделать-priorityqueue)
-   [`Stack`considered "obsolete". What is recommended to replace it with? Why?](#stack-считается-устаревшим-Чем-его-рекомендуют-заменять-Почему)
-   [Why is it needed?`HashMap`, if there is`Hashtable`?](#Зачем-нужен-hashmap-если-есть-hashtable)
-   [What is the difference between`HashMap`And`IdentityHashMap`? What is it for?`IdentityHashMap`?](#В-чем-разница-между-hashmap-и-identityhashmap-Для-чего-нужна-identityhashmap)
-   [What is the difference between`HashMap`And`WeakHashMap`? What is it used for?`WeakHashMap`?](#В-чем-разница-между-hashmap-и-weakhashmap-Для-чего-используется-weakhashmap)
-   [IN`WeakHashMap`WeakReferences are used. Why not create`SoftHashMap`на SoftReferences?](#В-weakhashmap-используются-weakreferences-А-почему-бы-не-создать-softhashmap-на-softreferences)
-   [IN`WeakHashMap`WeakReferences are used. Why not create`PhantomHashMap`on Phantom References?](#В-weakhashmap-используются-weakreferences-А-почему-бы-не-создать-phantomhashmap-на-phantomreferences)
-   [`LinkedHashMap`- what's in it`LinkedList`, and what from`HashMap`?](#linkedhashmap---что-в-нем-от-linkedlist-а-что-от-hashmap)
-   [What does “sorting” mean?`SortedMap`, except that`toString()`does it output all elements in order?](#В-чем-проявляется-сортированность-sortedmap-кроме-того-что-tostring-выводит-все-элементы-по-порядку)
-   [How it works`HashMap`?](#Как-устроен-hashmap)
-   [According to Knuth and Cormen, there are two main hash table implementations: open addressing based and chaining based. How it's implemented`HashMap`? Why do you think this particular implementation was chosen? What are the pros and cons of each approach?](#Согласно-Кнуту-и-Кормену-существует-две-основных-реализации-хэш-таблицы-на-основе-открытой-адресации-и-на-основе-метода-цепочек-Как-реализована-hashmap-Почему-по-вашему-мнению-была-выбрана-именно-эта-реализация-В-чем-плюсы-и-минусы-каждого-подхода)
-   [How it works`HashMap`when trying to save two elements into it using keys with the same`hashCode()`, but for which`equals() == false`?](#Как-работает-hashmap-при-попытке-сохранить-в-него-два-элемента-по-ключам-с-одинаковым-hashcode-но-для-которых-equals--false)
-   [What is the initial number of baskets in`HashMap`?](#Какое-начальное-количество-корзин-в-hashmap)
-   [What is the estimate of the time complexity of operations on elements from`HashMap`? Does it guarantee`HashMap`the specified complexity of selecting an element?](#Какова-оценка-временной-сложности-операций-над-элементами-из-hashmap-Гарантирует-ли-hashmap-указанную-сложность-выборки-элемента)
-   [Is it possible to have a situation where`HashMap`will degenerate into a list even with keys that have different`hashCode()`?](#Возможна-ли-ситуация-когда-hashmap-выродится-в-список-даже-с-ключами-имеющими-разные-hashcode)
-   [In what case can an element be lost in`HashMap`?](#В-каком-случае-может-быть-потерян-элемент-в-hashmap)
-   [Why can't you use`byte[]`as a key in`HashMap`?](#Почему-нельзя-использовать-byte-в-качестве-ключа-в-hashmap)
-   [What is the role`equals()`And`hashCode()`V`HashMap`?](#Какова-роль-equals-и-hashcode-в-hashmap)
-   [What is the maximum number of values`hashCode()`?](#Каково-максимальное-число-значений-hashcode)
-   [What is the worst running time of the get(key) method for a key that is not in`HashMap`?](#Какое-худшее-время-работы-метода-getkey-для-ключа-которого-нет-в-hashmap)
-   [What is the worst running time of the get(key) method for a key that is in`HashMap`?](#Какое-худшее-время-работы-метода-getkey-для-ключа-который-есть-в-hashmap)
-   [Why, despite the fact that the key is in`HashMap`not required to implement the interface`Comparable`, can a doubly linked list always be converted to a red-black-tree?](#Почему-несмотря-на-то-что-ключ-в-HashMap-не-обязан-реализовывать-интерфейс-Comparable-двусвязный-список-всегда-удается-преобразовать-в-красно-черное-дерево)
-   [How many transitions occur at the time of the call`HashMap.get(key)`by key that is in the table?](#Сколько-переходов-происходит-в-момент-вызова-hashmapgetkey-по-ключу-который-есть-в-таблице)
-   [How many new objects are created when you add a new element to`HashMap`?](#Сколько-создается-новых-объектов-когда-вы-добавляете-новый-элемент-в-hashmap)
-   [How and when does the number of baskets increase in`HashMap`?](#Как-и-когда-происходит-увеличение-количества-корзин-в-hashmap)
-   [Explain the meaning of parameters in a constructor`HashMap(int initialCapacity, float loadFactor)`.](#Объясните-смысл-параметров-в-конструкторе-hashmapint-initialcapacity-float-loadfactor)
-   [Will it work`HashMap`, if all added keys have the same`hashCode()`?](#Будет-ли-работать-hashmap-если-все-добавляемые-ключи-будут-иметь-одинаковый-hashcode)
-   [How to iterate through all the keys`Map`?](#Как-перебрать-все-ключи-map)
-   [How to loop through all values`Map`?](#Как-перебрать-все-значения-map)
-   [How to iterate through all key-value pairs in`Map`?](#Как-перебрать-все-пары-ключ-значение-в-map)
-   [What are the differences`TreeSet`And`HashSet`?](#В-чем-отличия-treeset-и-hashset)
-   [What happens if you add elements to`TreeSet`ascending?](#Что-будет-если-добавлять-элементы-в-treeset-по-возрастанию)
-   [How`LinkedHashSet`different from`HashSet`?](#Чем-linkedhashset-отличается-от-hashset)
-   [For`Enum`there is a special class`java.util.EnumSet`. For what? What the authors weren’t happy with`HashSet`or`TreeSet`?](#Для-enum-есть-специальный-класс-javautilenumset-Зачем-Чем-авторов-не-устраивал-hashset-или-treeset)
-   [What ways are there to iterate over the elements of a list?](#Какие-существуют-способы-перебирать-элементы-списка)
-   [How can I get synchronized objects of standard collections?](#Каким-образом-можно-получить-синхронизированные-объекты-стандартных-коллекций)
-   [How to get a read-only collection?](#Как-получить-коллекцию-только-для-чтения)
-   [Write a single threaded program that causes a collection to be thrown away`ConcurrentModificationException`.](#Напишите-однопоточную-программу-которая-заставляет-коллекцию-выбросить-concurrentmodificationexception)
-   [Give an example when some collection throws away`UnsupportedOperationException`.](#Приведите-пример-когда-какая-либо-коллекция-выбрасывает-unsupportedoperationexception)
-   [Implement the symmetric difference of two collections using the methods`Collection`(`addAll(...)`,`removeAll(...)`,`retainAll(...)`).](#Реализуйте-симметрическую-разность-двух-коллекций-используя-методы-collection-addall-removeall-retainall)
-   [How to make a cache with “invalidation policy” using LinkedHashMap?](#Как-используя-linkedhashmap-сделать-кэш-c-invalidation-policy)
-   [How to copy any elements in one line`collection`to an array?](#Как-одной-строчкой-скопировать-элементы-любой-collection-в-массив)
-   [As one call from`List`get`List`with all elements except the first and last 3?](#Как-одним-вызовом-из-list-получить-list-со-всеми-элементами-кроме-первых-и-последних-3-х)
-   [How to convert in one line`HashSet`V`ArrayList`?](#Как-одной-строчкой-преобразовать-hashset-в-arraylist)
-   [How to convert in one line`ArrayList`V`HashSet`?](#Как-одной-строчкой-преобразовать-arraylist-в-hashset)
-   [Do`HashSet`from the keys`HashMap`.](#Сделайте-hashset-из-ключей-hashmap)
-   [Do`HashMap`from`HashSet<Map.Entry<K, V>>`.](#Сделайте-hashmap-из-hashsetmapentryk-v)
