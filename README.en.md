...
-   [What innovations have appeared in Java 8 and JDK 8?](#Какие-нововведения-появились-в-java-8-и-jdk-8)
-   [What's happened_"lambda"_? What is the structure and usage features of a lambda expression?](#Что-такое-лямбда-Какова-структура-и-особенности-использования-лямбда-выражения)
-   [What variables can lambda expressions access?](#К-каким-переменным-есть-доступ-у-лямбда-выражений)
-   [How to sort a list of strings using a lambda expression?](#Как-отсортировать-список-строк-с-помощью-лямбда-выражения)
-   [What is a "method reference"?](#Что-такое-ссылка-на-метод)
-   [What types of method references do you know?](#Какие-виды-ссылок-на-методы-вы-знаете)
-   [Explain the expression`System.out::println`.](#Объясните-выражение-systemoutprintln)
-   [What are "functional interfaces"?](#Что-такое-функциональные-интерфейсы)
-   [What are functional interfaces for?`Function<T,R>`,`DoubleFunction<R>`,`IntFunction<R>`And`LongFunction<R>`?](#Для-чего-нужны-функциональные-интерфейсы-functiontr-doublefunctionr-intfunctionr-и-longfunctionr)
-   [What are functional interfaces for?`UnaryOperator<T>`,`DoubleUnaryOperator`,`IntUnaryOperator`And`LongUnaryOperator`?](#Для-чего-нужны-функциональные-интерфейсы-unaryoperatort-doubleunaryoperator-intunaryoperator-и-longunaryoperator)
-   [What are functional interfaces for?`BinaryOperator<T>`,`DoubleBinaryOperator`,`IntBinaryOperator`And`LongBinaryOperator`?](#Для-чего-нужны-функциональные-интерфейсы-binaryoperatort-doublebinaryoperator-intbinaryoperator-и-longbinaryoperator)
-   [What are functional interfaces for?`Predicate<T>`,`DoublePredicate`,`IntPredicate`And`LongPredicate`?](#Для-чего-нужны-функциональные-интерфейсы-predicatet-doublepredicate-intpredicate-и-longpredicate)
-   [What are functional interfaces for?`Consumer<T>`,`DoubleConsumer`,`IntConsumer`And`LongConsumer`?](#Для-чего-нужны-функциональные-интерфейсы-consumert-doubleconsumer-intconsumer-и-longconsumer)
-   [What are functional interfaces for?`Supplier<T>`,`BooleanSupplier`,`DoubleSupplier`,`IntSupplier`And`LongSupplier`?](#Для-чего-нужны-функциональные-интерфейсы-suppliert--booleansupplier-doublesupplier-intsupplier-и-longsupplier)
-   [Why do we need a functional interface?`BiConsumer<T,U>`?](#Для-чего-нужен-функциональный-интерфейс-biconsumertu)
-   [Why do we need a functional interface?`BiFunction<T,U,R>`?](#Для-чего-нужен-функциональный-интерфейс-bifunctiontur)
-   [Why do we need a functional interface?`BiPredicate<T,U>`?](#Для-чего-нужен-функциональный-интерфейс-bipredicatetu)
-   [Why do we need functional interfaces like`_To_Function`?](#Для-чего-нужны-функциональные-интерфейсы-вида-tofunction)
-   [What are functional interfaces for?`ToDoubleBiFunction<T,U>`,`ToIntBiFunction<T,U>`And`ToLongBiFunction<T,U>`?](#Для-чего-нужны-функциональные-интерфейсы-todoublebifunctiontu-tointbifunctiontu-и-tolongbifunctiontu)
-   [What are functional interfaces for?`ToDoubleFunction<T>`,`ToIntFunction<T>`And`ToLongFunction<T>`?](#Для-чего-нужны-функциональные-интерфейсы-todoublefunctiont-tointfunctiont-и-tolongfunctiont)
-   [What are functional interfaces for?`ObjDoubleConsumer<T>`,`ObjIntConsumer<T>`And`ObjLongConsumer<T>`?](#Для-чего-нужны-функциональные-интерфейсы-objdoubleconsumert-objintconsumert-и-objlongconsumert)
-   [What's happened`StringJoiner`?](#Что-такое-stringjoiner)
-   [What's happened`default`interface methods?](#Что-такое-default-методы-интрефейса)
-   [How to call`default`interface method in the class that implements this interface?](#Как-вызывать-default-метод-интерфейса-в-реализующем-этот-интерфейс-классе)
-   [What's happened`static`interface method?](#Что-такое-static-метод-интерфейса)
-   [How to call`static`interface method?](#Как-вызывать-static-метод-интерфейса)
-   [What's happened`Optional`?](#Что-такое-optional)
-   [What's happened`Stream`?](#Что-такое-stream)
-   [What are the different ways to create a stream?](#Какие-существуют-способы-создания-стрима)
-   [What is the difference between`Collection`And`Stream`?](#В-чем-разница-между-collection-и-stream)
-   [What is the method for?`collect()`in restraints?](#Для-чего-нужен-метод-collect-в-стримах)
-   [Why are methods used in streams?`forEach()`And`forEachOrdered()`?](#Для-чего-в-стримах-применяются-методы-foreach-и-foreachordered)
-   [What are the methods used in streams?`map()`And`mapToInt()`,`mapToDouble()`,`mapToLong()`?](#Для-чего-в-стримах-предназначены-методы-map-и-maptoint-maptodouble-maptolong)
-   [What is the purpose of the method`filter()`in restraints?](#Какова-цель-метода-filter-в-стримах)
-   [What is the method used for in streams?`limit()`?](#Для-чего-в-стримах-предназначен-метод-limit)
-   [What is the method used for in streams?`sorted()`?](#Для-чего-в-стримах-предназначен-метод-sorted)
-   [What are the methods used in streams?`flatMap()`,`flatMapToInt()`,`flatMapToDouble()`,`flatMapToLong()`?](#Для-чего-в-стримах-предназначены-методы-flatmap-flatmaptoint-flatmaptodouble-flatmaptolong)
-   [Explain about parallel processing in Java 8.](#Расскажите-о-параллельной-обработке-в-java-8)
-   [What ultimate methods of working with streams do you know?](#Какие-конечные-методы-работы-со-стримами-вы-знаете)
-   [What intermediate methods of working with streams do you know?](#Какие-промежуточные-методы-работы-со-стримами-вы-знаете)
-   [How to display 10 random numbers using`forEach()`?](#Как-вывести-на-экран-10-случайных-чисел-используя-foreach)
-   [How can you display unique squares of numbers using the method`map()`?](#Как-можно-вывести-на-экран-уникальные-квадраты-чисел-используя-метод-map)
-   [How to display the number of empty lines using the method`filter()`?](#Как-вывести-на-экран-количество-пустых-строк-с-помощью-метода-filter)
-   [How to display 10 random numbers in ascending order?](#Как-вывести-на-экран-10-случайных-чисел-в-порядке-возрастания)
-   [How to find the maximum number in a set?](#Как-найти-максимальное-число-в-наборе)
-   [How to find the minimum number in a set?](#Как-найти-минимальное-число-в-наборе)
-   [How to get the sum of all numbers in a set?](#Как-получить-сумму-всех-чисел-в-наборе)
-   [How to get the average of all numbers?](#Как-получить-среднее-значение-всех-чисел)
-   [What additional methods for working with associative arrays (maps) appeared in Java 8?](#Какие-дополнительные-методы-для-работы-с-ассоциативными-массивами-maps-появились-в-java-8)
-   [What's happened`LocalDateTime`?](#Что-такое-localdatetime)
-   [What's happened`ZonedDateTime`?](#Что-такое-zoneddatetime)
-   [How to get current date using Date Time API from Java 8?](#Как-получить-текущую-дату-с-использованием-date-time-api-из-java-8)
-   [How to add 1 week, 1 month, 1 year, 10 years to current date using Date Time API?](#Как-добавить-1-неделю-1-месяц-1-год-10-лет-к-текущей-дате-с-использованием-date-time-api)
-   [How to get next Tuesday using Date Time API?](#Как-получить-следующий-вторник-используя-date-time-api)
-   [How to get the second Saturday of the current month using the Date Time API?](#Как-получить-вторую-субботу-текущего-месяца-используя-date-time-api)
-   [How to get the current time accurate to milliseconds using the Date Time API?](#Как-получить-текущее-время-с-точностью-до-миллисекунд-используя-date-time-api)
-   [How to get the current local time accurate to milliseconds using the Date Time API?](#Как-получить-текущее-время-по-местному-времени-с-точностью-до-миллисекунд-используя-date-time-api)
-   [How to define a repeatable annotation?](#Как-определить-повторяемую-аннотацию)
-   [What's happened`Nashorn`?](#Что-такое-nashorn)
-   [What's happened`jjs`?](#Что-такое-jjs)
-   [What class appeared in Java 8 for encoding/decoding data?](#Какой-класс-появился-в-java-8-для-кодированиядекодирования-данных)
-   [How to create a Base64 encoder and decoder?](#Как-создать-base64-кодировщик-и-декодировщик)
