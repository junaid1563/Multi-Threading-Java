...
-   [What is the difference between JRE, JVM and JDK?](#Чем-различаются-jre-jvm-и-jdk)
-   [What access modifiers are there?](#Какие-существуют-модификаторы-доступа)
-   [What does the keyword mean?`final`?](#О-чем-говорит-ключевое-слово-final)
-   [What values ​​are default variables initialized to?](#Какими-значениями-инициализируются-переменные-по-умолчанию)
-   [What do you know about the function`main()`?](#Что-вы-знаете-о-функции-main)
-   [What logical operations and operators do you know?](#Какие-логические-операции-и-операторы-вы-знаете)
-   [What is the ternary select operator?](#Что-такое-тернарный-оператор-выбора)
-   [What bitwise operations do you know?](#Какие-побитовые-операции-вы-знаете)
-   [Where and for what is the modifier used?`abstract`?](#Где-и-для-чего-используется-модификатор-abstract)
-   [Define the concept_"interface"_. What default modifiers do fields and interface methods have?](#Дайте-определение-понятию-интерфейс-Какие-модификаторы-по-умолчанию-имеют-поля-и-методы-интерфейсов)
-   [How is an abstract class different from an interface? When should you use an abstract class and when should you use an interface?](#Чем-абстрактный-класс-отличается-от-интерфейса-В-каких-случаях-следует-использовать-абстрактный-класс-а-в-каких-интерфейс)
-   [Why do some interfaces not define methods at all?](#Почему-в-некоторых-интерфейсах-вообще-не-определяют-методов)
-   [Why can't you declare an interface method with a modifier?`final`?](#Почему-нельзя-объявить-метод-интерфейса-с-модификатором-final)
-   [Which has a higher level of abstraction -_Class_,_abstract class_or_interface_?](#Что-имеет-более-высокий-уровень-абстракции---класс-абстрактный-класс-или-интерфейс)
-   [Can an object access a member of a class declared as`private`? If yes, then how?](#Может-ли-объект-получить-доступ-к-члену-класса-объявленному-как-private-Если-да-то-каким-образом)
-   [What is the order in which constructors and initialization blocks are called, taking into account the class hierarchy?](#Каков-порядок-вызова-конструкторов-и-блоков-инициализации-с-учётом-иерархии-классов)
-   [Why are initialization blocks needed and what are they?](#Зачем-нужны-и-какие-бывают-блоки-инициализации)
-   [Which Java constructs can the modifier apply to?`static`?](#К-каким-конструкциям-java-применим-модификатор-static)
-   [Why are static initialization blocks used in Java?](#Для-чего-в-java-используются-статические-блоки-инициализации)
-   [What happens if an exception occurs in the initialization block?](#Что-произойдёт-если-в-блоке-инициализации-возникнет-исключительная-ситуация)
-   [What exception is thrown when an error occurs in a class initialization block?](#Какое-исключение-выбрасывается-при-возникновении-ошибки-в-блоке-инициализации-класса)
-   [Can a static method be overridden or overloaded?](#Может-ли-статический-метод-быть-переопределён-или-перегружен)
-   [Can non-static methods overload static ones?](#Могут-ли-нестатические-методы-перегрузить-статические)
-   [Is it possible to narrow the access level/return type when overriding a method?](#Можно-ли-сузить-уровень-доступатип-возвращаемого-значения-при-переопределении-метода)
-   [When overriding a method, is it possible to change: the access modifier, the return type, the type of the argument or their number, the names of the arguments or their order; remove, add, change the order of section elements`throws`?](#Возможно-ли-при-переопределении-метода-изменить-модификатор-доступа-возвращаемый-тип-тип-аргумента-или-их-количество-имена-аргументов-или-их-порядок-убирать-добавлять-изменять-порядок-следования-элементов-секции-throws)
-   [How to access overridden methods of a parent class?](#Как-получить-доступ-к-переопределенным-методам-родительского-класса)
-   [Is it possible to declare a method abstract and static at the same time?](#Можно-ли-объявить-метод-абстрактным-и-статическим-одновременно)
-   [What is the difference between a class instance member and a static class member?](#В-чем-разница-между-членом-экземпляра-класса-и-статическим-членом-класса)
-   [Where is initialization of static/non-static fields allowed?](#Где-разрешена-инициализация-статическихнестатических-полей)
-   [What types of classes are there in java?](#Какие-типы-классов-бывают-в-java)
-   [Tell us about nested classes. In what cases are they used?](#Расскажите-про-вложенные-классы-В-каких-случаях-они-применяются)
-   [What's happened_"static class"_?](#Что-такое-статический-класс)
-   [What are the features of using nested classes: static and internal? What is the difference between them?](#Какие-существуют-особенности-использования-вложенных-классов-статических-и-внутренних-В-чем-заключается-разница-между-ними)
-   [What's happened_"local class"_? What are its features?](#Что-такое-локальный-класс-Каковы-его-особенности)
-   [What's happened_"anonymous classes"_? Where are they used?](#Что-такое-анонимные-классы-Где-они-применяются)
-   [How can I access a field in an outer class from a nested class?](#Каким-образом-из-вложенного-класса-получить-доступ-к-полю-внешнего-класса)
-   [What is the operator used for?`assert`?](#Для-чего-используется-оператор-assert)
-   [What's happened_Heap_And_Stack_memory in Java? What's the difference between them?](#Что-такое-heap-и-stack-память-в-java-Какая-разница-между-ними)
-   [Is it true that primitive data types are always stored on the stack, and instances of reference data types are always stored on the heap?](#Верно-ли-утверждение-что-примитивные-типы-данных-всегда-хранятся-в-стеке-а-экземпляры-ссылочных-типов-данных-в-куче)
-   [How are variables passed to methods, by value or by reference?](#Каким-образом-передаются-переменные-в-методы-по-значению-или-по-ссылке)
-   [What is a garbage collector for?](#Для-чего-нужен-сборщик-мусора)
-   [How does the garbage collector work?](#Как-работает-сборщик-мусора)
-   [What types of garbage collectors are implemented in the HotSpot virtual machine?](#Какие-разновидности-сборщиков-мусора-реализованы-в-виртуальной-машине-hotspot)
-   [Describe the algorithm of operation of a garbage collector implemented in the HotSpot virtual machine.](#Опишите-алгоритм-работы-какого-нибудь-сборщика-мусора-реализованного-в-виртуальной-машине-hotspot)
-   [What is a "string pool"?](#Что-такое-пул-строк)
-   [What's happened`finalize()`? Why is it needed?](#Что-такое-finalize-Зачем-он-нужен)
-   [What happens to the garbage collector if the method is executed`finalize()`does it take a noticeably long time, or will an exception be thrown during execution?](#Что-произойдет-со-сборщиком-мусора-если-выполнение-метода-finalize-требует-ощутимо-много-времени-или-в-процессе-выполнения-будет-выброшено-исключение)
-   [How are they different?`final`,`finally`And`finalize()`?](#Чем-отличаются-final-finally-и-finalize)
-   [Tell us about type casting. What is type demotion and promotion?](#Расскажите-про-приведение-типов-Что-такое-понижение-и-повышение-типа)
-   [When an application might throw an exception`ClassCastException`?](#Когда-в-приложении-может-быть-выброшено-исключение-classcastexception)
-   [What are literals?](#Что-такое-литералы)
-   [What's happened_autoboxing_in Java and what are the rules for packing primitive types into wrapper classes?](#Что-такое-autoboxing-автоупаковка-в-java-и-каковы-правила-упаковки-примитивных-типов-в-классы-обертки)
-   [What are the features of the class?`String`?](#Какие-есть-особенности-класса-string)
-   [Why`String`immutable and finalized class?](#Почему-string-неизменяемый-и-финализированный-класс)
-   [Why`char[]`preferable`String`to store the password?](#Почему-char-предпочтительнее-string-для-хранения-пароля)
-   [Why is string a popular key in`HashMap`in Java?](#Почему-строка-является-популярным-ключом-в-hashmap-в-java)
-   [What does the method do?`intern()`in class`String`?.](#Что-делает-метод-intern-в-классе-string)
-   [Is it possible to use strings in a design`switch`?](#Можно-ли-использовать-строки-в-конструкции-switch)
-   [What is the main difference between`String`,`StringBuffer`,`StringBuilder`?](#Какая-основная-разница-между-string-stringbuffer-stringbuilder)
-   [What is a class`Object`? What methods does it have?](#Что-такое-класс-object-Какие-в-нем-есть-методы)
-   [Define the concept of “constructor”.](#Дайте-определение-понятию-конструктор)
-   [What's happened_"default constructor"_?](#Что-такое-конструктор-по-умолчанию)
-   [What is the difference between a default constructor, a copy constructor, and a constructor with parameters?](#Чем-отличаются-конструктор-по-умолчанию-конструктор-копирования-и-конструктор-с-параметрами)
-   [Where and how can you use a private constructor?](#Где-и-как-вы-можете-использовать-приватный-конструктор)
-   [Tell us about loader classes and dynamic class loading.](#Расскажите-про-классы-загрузчики-и-про-динамическую-загрузку-классов)
-   [What's happened_Reflection_?](#Что-такое-reflection)
-   [Why is it needed?`equals()`. How is it different from surgery?`==`?](#Зачем-нужен-equals-Чем-он-отличается-от-операции-)
-   [If you want to override`equals()`, what conditions must be met?](#Если-вы-хотите-переопределить-equals-какие-условия-должны-выполняться)
-   [What properties does the generated`equals()`equivalence relation?](#Какими-свойствами-обладает-порождаемое-equals-отношение-эквивалентности)
-   [Method overriding rules`Object.equals()`.](#Правила-переопределения-метода-objectequals)
-   [What is the connection between`hashCode()`And`equals()`?](#Какая-связь-между-hashcode-и-equals)
-   [If`equals()`overridden, are there any other methods that should be overridden?](#Если-equals-переопределен-есть-ли-какие-либо-другие-методы-которые-следует-переопределить)
-   [What happens if you override`equals()`without overriding`hashCode()`? What problems might arise?](#Что-будет-если-переопределить-equals-не-переопределяя-hashcode-Какие-могут-возникнуть-проблемы)
-   [How are the methods implemented?`hashCode()`And`equals()`in class`Object`?](#Каким-образом-реализованы-методы-hashcode-и-equals-в-классе-object)
-   [What is the method for?`hashCode()`?](#Для-чего-нужен-метод-hashcode)
-   [What are the rules for method overriding`Object.hashCode()`?](#Каковы-правила-переопределения-метода-objecthashcode)
-   [Are there any guidelines on what fields should be used when counting`hashCode()`?](#Есть-ли-какие-либо-рекомендации-о-том-какие-поля-следует-использовать-при-подсчете-hashcode)
-   [Can different objects have the same`hashCode()`?](#Могут-ли-у-разных-объектов-быть-одинаковые-hashcode)
-   [If the class`Point{int x, y;}`implement the method`equals(Object that) {(return this.x == that.x && this.y == that.y)}`, but make the hash code in the form`int hashCode() {return x;}`, then will such points be correctly placed and extracted from`HashSet`?](#Если-у-класса-pointint-x-y-реализовать-метод-equalsobject-that-return-thisx--thatx--thisy--thaty-но-сделать-хэш-код-в-виде-int-hashcode-return-x-то-будут-ли-корректно-такие-точки-помещаться-и-извлекаться-из-hashset)
-   [Can different objects`(ref0 != ref1)`be`ref0.equals(ref1) == true`?](#Могут-ли-у-разных-объектов-ref0--ref1-быть-ref0equalsref1--true)
-   [Can different references to the same object`(ref0 == ref1)`be`ref0.equals(ref1) == false`?](#Могут-ли-у-разных-ссылок-на-один-объект-ref0--ref1-быть-ref0equalsref1--false)
-   [Is it possible to implement the method like this?`equals(Object that) {return this.hashCode() == that.hashCode()}`?](#Можно-ли-так-реализовать-метод-equalsobject-that-return-thishashcode--thathashcode)
-   [IN`equals()`you need to check that the argument`equals(Object that)`the same type as the object itself. What is the difference between`this.getClass() == that.getClass()`And`that instanceof MyClass`?](#В-equals-требуется-проверять-что-аргумент-equalsobject-that-такого-же-типа-что-и-сам-объект-В-чем-разница-между-thisgetclass--thatgetclass-и-that-instanceof-myclass)
-   [Is it possible to implement the method`equals()`class`MyClass`like this:`class MyClass {public boolean equals(MyClass that) {return this == that;}}`?](#Можно-ли-реализовать-метод-equals-класса-myclass-вот-так-class-myclass-public-boolean-equalsmyclass-that-return-this--that)
-   [There is a class`Point{int x, y;}`. Why hash code in the form`31 * x + y`preferable to`x + y`?](#Есть-класс-pointint-x-y-Почему-хэш-код-в-виде-31--x--y-предпочтительнее-чем-x--y)
-   [Tell us about cloning objects.](#Расскажите-про-клонирование-объектов)
-   [What is the difference between_superficial_And_deep_cloning?](#В-чем-отличие-между-поверхностным-и-глубоким-клонированием)
-   [Which cloning method is preferable?](#Какой-способ-клонирования-предпочтительней)
-   [Why method`clone()`declared in class`Object`, not in the interface`Cloneable`?](#Почему-метод-clone-объявлен-в-классе-object-а-не-в-интерфейсе-cloneable)
-   [Describe the hierarchy of exceptions.](#Опишите-иерархию-исключений)
-   [What types of exceptions in Java do you know, how do they differ?](#Какие-виды-исключений-в-java-вы-знаете-чем-они-отличаются)
-   [What's happened_checked_And_unchecked exception_?](#Что-такое-checked-и-unchecked-exception)
-   [Which operator allows you to force an exception to be thrown?](#Какой-оператор-позволяет-принудительно-выбросить-исключение)
-   [What does the keyword mean?`throws`?](#О-чем-говорит-ключевое-слово-throws)
-   [How to write your own ("custom") exception?](#Как-написать-собственное-пользовательское-исключение)
-   [Which ones exist?_unchecked exception_?](#Какие-существуют-unchecked-exception)
-   [What are class errors?`Error`?](#Что-представляет-из-себя-ошибки-класса-error)
-   [What do you know about`OutOfMemoryError`?](#Что-вы-знаете-о-outofmemoryerror)
-   [Describe the operation of the block_try-catch-finally_.](#Опишите-работу-блока-try-catch-finally)
-   [What is mechanism_try-with-resources_?](#Что-такое-механизм-try-with-resources)
-   [Is it possible to use a block_try-finally_(without`catch`)?](#Возможно-ли-использование-блока-try-finally-без-catch)
-   [Can one block`catch`catch multiple exceptions at once?](#Может-ли-один-блок-catch-отлавливать-сразу-несколько-исключений)
-   [Is the block always executed?`finally`?](#Всегда-ли-исполняется-блок-finally)
-   [Are there situations where a block`finally`will not be fulfilled?](#Существуют-ли-ситуации-когда-блок-finally-не-будет-выполнен)
-   [Can the method_main()_throw an exception externally and if so, where will this exception be processed?](#Может-ли-метод-main-выбросить-исключение-во-вне-и-если-да-то-где-будет-происходить-обработка-данного-исключения)
-   [Suppose there is a method that can throw`IOException`And`FileNotFoundException`in what order should the blocks go?`catch`? How many blocks`catch`will it be done?](#Предположим-есть-метод-который-может-выбросить-ioexception-и-filenotfoundexception-в-какой-последовательности-должны-идти-блоки-catch-Сколько-блоков-catch-будет-выполнено)
-   [What's happened_generics_?](#Что-такое-generics)
-   [What's happened_"internationalization"_,_"localization"_?](#Что-такое-интернационализация-локализация)
